...
class ListservMessageParser(email.parser.Parser):
    ...
    def create_email_message(
        ...
        **header,
        ...
        # TODO *1: Not all messages in one mailing list have the same keys, e.g.,
        # for 3GPP_TSG_SA_WG4 'Content-Transfer-Encoding' and 'MIME-Version' are
        # not defined for all messages...
                ...
                if update:
                    for mlist_url, mlist_file in zip(mlist_urls, mlist_files):
                        ...
                        last_entry_dt = ListservList.from_mbox(
                            name=name,
                            filepath=mlist_file,
                        ).to_pandas_dataframe(include_body=False)["date"].max()
